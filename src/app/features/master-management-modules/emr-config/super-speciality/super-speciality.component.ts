...
export class SuperSpecialityComponent implements OnInit {
  ...
  apiAdd="mstSuperSpeciality/create";
  apiUpdate="mstSuperSpeciality/update";
  apidelete="mstSuperSpeciality/inActivate";
  apiactive="mstSuperSpeciality/activate";
  ...
  isActive(data:any){
    if(!data.isActive){
      this.http.reactiveData(this.apiactive, data, data.ssId)
        .subscribe(b_Data => {
          this.data = undefined;
          this.getAllSs();
        })
  ...
  deleteSs(ssData: any) {
    this.http.deleteData(this.apidelete, ssData.ssId)
      .subscribe(data => {
        this.data = undefined;
        this.getAllSs();
        console.log("data" + data)
